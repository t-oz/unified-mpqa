...
class Vocab(object):
    ...
    def __init__(self, word_counter, srltag_counter, label_counter, min_occur_count=2):
        self._id2word = ['<pad>', '<unk>']
        self._wordid2freq = [10000, 10000]
        self._id2extword = ['<pad>', '<unk>']
        self._id2srltag = ['<pad>', '<unk>']
        self._id2label = ['<pad>']
        ...
        for srltag, count in srltag_counter.most_common():
            self._id2srltag.append(srltag)
        ...
        self._srltag2id = reverse(self._id2srltag)
        if len(self._srltag2id) != len(self._id2srltag):
            print("serious bug: srl tags dumplicated, please check!")
    ...
    def srltag2id(self, xs):
        if isinstance(xs, list):
            return [self._srltag2id.get(x, self.UNK) for x in xs]
        return self._srltag2id.get(xs, self.UNK)

    def id2srltag(self, xs):
        if isinstance(xs, list):
            return [self._id2srltag[x] for x in xs]
        return self._id2srltag[xs]
    ...
    @property
    def srlvocab_size(self):
        return len(self._id2srltag)
    ...
    with open(corpusFile, 'r') as infile:
        for sentence in readSRL(infile):
            ...
            for token in sentence.words:
                ...
                srltag_counter[token.srltag] += 1
                if index < sentence.key_start or index > sentence.key_end:
                    label_counter[token.label] += 1
                index = index + 1

    return Vocab(word_counter, srltag_counter, label_counter, min_occur_count)