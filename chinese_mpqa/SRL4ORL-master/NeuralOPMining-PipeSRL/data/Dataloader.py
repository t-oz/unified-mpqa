...
def batch_data_variable(batch, vocab):
    ...
    srltags = Variable(torch.LongTensor(batch_size, length).fill_(vocab.PAD), requires_grad=False)
    ...
    for tokens, key_head, key_start, key_end in sentences_numberize(batch, vocab):
        ...
        for word in tokens:
            ...
            srltags[b, index] = word[2]
            labels[b, index] = word[3]
            inmasks[b, index] = 1
            outmasks[b, index] = 1
            predicts[b, index] = 2
    ...
    return words, extwords, srltags, predicts, inmasks, labels, outmasks.byte()
...
def batch_variable_srl(inputs, labels, vocab):
    for input, label in zip(inputs, labels):
        ...
        tokens = []
        for idx in range(input.length):
            tokens.append(Word(idx, input.words[idx].org_form, input.words[idx].srltag, normed_labels[idx]))
